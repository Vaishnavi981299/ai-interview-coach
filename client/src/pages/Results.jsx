    ...
    useEffect(() => {
        async function fetchSession() {
            try {
                ...
                const aiMessages = response.data.session.messages.filter(m => m.role === 'ai');
                ...
                if (scores.length > 0) {
                    const avg = parseFloat((scores.reduce((sum, s) => sum + s, 0) / scores.length).toFixed(1));
                    await completeSession({ sessionId, finalScore: avg });
                }
            } catch(error) {
